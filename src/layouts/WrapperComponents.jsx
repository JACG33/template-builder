...
export const WrapperComponents = ({ children, secctionName }) => {
  ...
  return (
    <div className={`py-2 px-4 border-b-2 ${openSection == true ? "bg-slate-300 text-slate-600" : ""}`}>
      <div className='flex justify-between items-center rounded-lg hover:bg-slate-400 hover:shadow-sm transition cursor-pointer' onClick={handleOpen}>
      ...
      <div className={`grid overflow-auto ${openSection == true ? "h-full bg-slate-300 mt-2" : "h-0"}`}>
        {children}
      </div>
    </div>
  )
