...
export function EditorProvider({ children }) {
    ...
    if (openEditor == true) {
      if (
        ...
        setConfigComponent({
          ...configComponent,
          mediaQuerys: {
            ...configComponent.mediaQuerys,
            [breackPoint]: {
              ...configComponent.mediaQuerys[breackPoint],
              [actualConfig.nameConfig]: { ...conf },
            },
          },
        });
        cssStylesRef.current = {
          ...cssStylesRef.current,
          mediaQuerys: {
            ...cssStylesRef.current.mediaQuerys,
            [breackPoint]: {
              ...cssStylesRef.current.mediaQuerys[breackPoint],
              [actualConfig.nameConfig]: { ...conf },
            },
          },
        };
        ...
        cssStylesRef.current = {
          ...configComponent,
          normalStyles: {
            ...cssStylesRef.current.normalStyles,
            [actualConfig.nameConfig]: conf,
          },
        };
        setHeadStyles();
  ...
  const makeCssRule = ({ toIterate = [] }) => {
    let rule = ``;
    for (const iterator in toIterate)
      toIterate[iterator] != ""
        ? (rule += `  ${joinAndLower(iterator)}: ${toIterate[iterator]};\n`)
        : "";
    return rule;
  };
    ...
    if (Object.keys(configComponent.normalStyles).length == 0) {
      ...
      if (configComponent) {
        ...
        normalClasses?.forEach((classStyle) => {
          ...
          if (
            Object.keys(cssStylesRef.current.normalStyles[classStyle]).length >
            0
          ) {
            cssStyles += `\n.${classStyle} {\n`;
            cssStyles += makeCssRule({
              toIterate: cssStylesRef.current.normalStyles[classStyle],
            });
            cssStyles += `}\n`;
          }
        });
        ...
        if (mediaQuerysClasses.length > 0) {
          mediaQuerysClasses?.forEach((mediaQueryClass) => {
            ...
            let innerClasses = Object.keys(
              cssStylesRef.current.mediaQuerys[mediaQueryClass]
            );
  ...
  const setUiStyles = ({ uiStyles, uiStylesMediaquerys, scripts }) => {
    setConfigComponent({
      mediaQuerys: {
        ...configComponent.mediaQuerys,
        mobilex2: {
          ...configComponent.mediaQuerys?.mobilex2,
          ...uiStylesMediaquerys?.mobilex2,
        },
        tablet: {
          ...configComponent.mediaQuerys?.tablet,
          ...uiStylesMediaquerys?.tablet,
        },
        desktop: {
          ...configComponent.mediaQuerys?.desktop,
          ...uiStylesMediaquerys?.desktop,
        },
        desktopx2: {
          ...configComponent.mediaQuerys?.desktopx2,
          ...uiStylesMediaquerys?.desktopx2,
        },
        desktopx3: {
          ...configComponent.mediaQuerys?.desktopx3,
          ...uiStylesMediaquerys?.desktopx3,
        },
      },
    ...
    cssStylesRef.current = {
      mediaQuerys: {
        ...cssStylesRef.current.mediaQuerys,
        mobilex2: {
          ...cssStylesRef.current.mediaQuerys?.mobilex2,
          ...uiStylesMediaquerys?.mobilex2,
        },
        tablet: {
          ...cssStylesRef.current.mediaQuerys?.tablet,
          ...uiStylesMediaquerys?.tablet,
        },
        desktop: {
          ...cssStylesRef.current.mediaQuerys?.desktop,
          ...uiStylesMediaquerys?.desktop,
        },
        desktopx2: {
          ...cssStylesRef.current.mediaQuerys?.desktopx2,
          ...uiStylesMediaquerys?.desktopx2,
        },
        desktopx3: {
          ...cssStylesRef.current.mediaQuerys?.desktopx3,
          ...uiStylesMediaquerys?.desktopx3,
        },
      },
      normalStyles: { ...configComponent.normalStyles, ...uiStyles },
    ...
    ids.forEach((ele) => {
      ...
      keysScripts.forEach((key) => {
        if (
          ...
          Object.keys(scripts[key]).length == 0
        )
          delete scripts[key];
        else delete scripts[key]?.[ele];
      });
  ...
  const getConfigComponent = (id) => {
    if (
      ...
      return configComponent.mediaQuerys[breackPoint]?.[id]
        ? configComponent.mediaQuerys[breackPoint][id]
        : {};
    } else {
      return configComponent.normalStyles?.[id]
        ? configComponent.normalStyles[id]
        : {};
    }
  };
